...
router.post('/panier', (req, res) => {
  ...
  const indexArt = articles.findIndex(art => {
    return art.id === id;
  })
  
  if(indexArt == -1){
    res.status(400).json({message: "article can't be find"})
    return
  }

  const indexPan = req.session.panier.articles.findIndex(art2 => {
    return art2.id === id;
  })

  if( indexPan != -1){
    res.status(400).json({message: "article already in the cart"})
    return
  }

  if(quantity <= 0){
    res.status(400).json({message: "quantity can not be negative"})
    return
  }
  ...
  req.session.panier.articles.push(achat)
...
router.put('/panier', (req, res) => {
  ...
  if( indexPan == -1){
    res.status(400).json({message: "article not in the cart"})
    return
  }
  ...
  req.session.panier.articles.splice(indexPan,1,quantity)
  // on envoie l'article ajouté à l'utilisateur
  res.status(200)
  res.json(achat)
})
